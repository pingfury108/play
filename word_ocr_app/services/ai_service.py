...
输出严格的 JSON 格式：
{
  ...
  "examples": [
    {
      "seq_num": 1,
      "original_text": "6. What did some customers think...",
      "source": "四级2025-6-第2套 听力"
    }
  ]
}
...
3. **examples 数组（严格按图片从上到下顺序）**：
   - **seq_num**：图片内的顺序编号（1, 2, 3...），第1条例句=1，第2条=2
   - **original_text**：提取完整的英文原文（必须包含题号！）
     - **重要**：保留题号，如 "6. What did..."、"21. What has..."
     - 保留原文的所有标点符号，不要修改
     - 如果是选择题，保留题目和所有选项 A/B/C/D
     - 去除来源信息（不要包含"四级 2025-6-第 2 套 听力"）
   - **source**：提取来源信息，格式化为"四级YYYY-M-第N套 类型"
     - 类型包括：听力、阅读、写作、翻译

## 关键要求
1. **从上到下**：examples 数组必须严格按照图片中例句的从上到下顺序排列
2. **原文必须含题号**：original_text 必须包含题号（如 "6. What did..."）
3. **来源分离**：source 单独提取，不要放在原文中
4. **标点保留**：保留原文的所有标点符号，不做任何修改"""
