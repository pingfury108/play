...
2. **cet4_count**：提取"在历年真题中出现 X 次"中的数字 X（这是该单词总共出现的次数）

3. **examples 数组（严格按图片从上到下顺序）**：
   - **seq_num**：按顺序编号（第1条=1, 第2条=2, 第3条=3...）
     - ⚠️ 注意：不要使用图片中的真题题号（如6、7、21），使用1,2,3的顺序号
   - **original_text**：提取完整的英文原文（必须包含题号）
     - 保留题号，如 "6. What did some customers..."、"21. What has become..."
     - 保留原文的所有标点符号，不要修改
     - 如果是选择题，保留题目和所有选项 A/B/C/D
     - 保留完整的句子或段落
     - 去除来源信息（如"四级 2025-6-第 2 套 听力"，不要放在原文中）
   - **source**：提取来源信息，格式化为"四级YYYY-M-第N套 类型"
     - 从每条例句底部提取，如"四级 2025-6-第 2 套 听力"→"四级2025-6-第2套 听力"
     - 类型包括：听力、阅读、写作、翻译

## 关键要求（必须遵守）
1. **顺序**：examples 数组必须严格按照图片中例句的从上到下顺序排列
2. **题号**：seq_num 使用图片中的真题题号，不要重新排序
3. **原文**：保留原文的所有标点符号，不做任何修改
4. **出现次数**：cet4_count 是单词总出现次数（如 28），与单个例句无关"""
