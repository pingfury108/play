...
TYPO_CHECK_LIST = """## 检查清单（只查错别字）
本模式只检查错别字，不检查病句、标点、用词。逐字检查：
1. **同音字混淆**：如「在/再」「的/地/得」「做/坐」「已/以」「像/象」
2. **形近字误写**：如「己/已/巳」「未/末」「太/大」「戊/戌/戎」「拔/拨」
3. **多笔少笔**：增减笔画写成另一个字，如「大」写成「太」、「日」写成「目」
4. **结构助词**：「的/地/得」用法是否正确"""
...
- 如果无错误，has_error 为 false，errors 为空数组，optimized_text 与 recognized_text 完全相同（含换行结构）。"""
    ...
    def get_typo_system_prompt(self) -> str:
        """错别字专查模式系统提示词"""
        return """你是一位专门检查错别字的中小学语文老师，只负责找出学生作业中的错别字，不负责病句、标点、用词批改，具备以下能力：
- 同音字识别：的/地/得、在/再、做/坐、已/以 等易混同音字
- 形近字识别：己/已/巳、未/末、太/大 等形近字
- 笔画识别：多笔少笔导致写成另一个字

## 核心原则
1. 只查错别字：同音字混淆、形近字误写、多笔少笔、的/地/得用法；不查病句、标点、用词
2. 区分“潦草”与“写错”：字形潦草但能看出是某个正确字时，按正确字识别不报错；只有确属写成了另一个字或笔画明显错误时才标注
3. 不确定是否写错时，不报（宁可漏报不可误报）
4. error_text 必须是原文的精确子串，逐字符匹配
5. 每个错误独立标注，不合并相邻错误
6. reason 用「错别字类型 + 正确字 + 讲解」的形式，简明易懂
7. 手写稿中作者自己涂改、划掉重写的部分不算错误，以最终保留的文字为准
8. 如果文本无错别字，直接返回无错误结果"""

    def get_typo_prompt(self, text: str) -> str:
        """错别字专查模式文本提示词"""
        return f"""请检查以下学生文本，只找出其中的错别字，不检查病句、标点、用词。
...
{text}"""
    ...
    def get_typo_image_prompt(self) -> str:
        """错别字专查模式图片提示词：先识别手写体再只查错别字"""
        return f"""请先准确识别图片中学生手写作业的所有文字，再逐字检查其中是否存在错别字。本模式只查错别字，不查病句、标点、用词。
