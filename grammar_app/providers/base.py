...
按以下维度逐项检查，不要遗漏：
1. **错别字**：同音字混淆（如「在/再」「做/坐」「已/以」）
2. **的、地、得误用**：「的/地/得」结构助词用法是否正确
3. **多字、少字**：句子中多余的字（赘余）或遗漏的字
4. **成分残缺**：主语、谓语、宾语等句子成分缺失
5. **标点符号使用错误**：标点本身用错（如该用句号却用逗号、顿号与逗号混用、问号感叹号错用）
6. **断句错误**：一逗到底、该断句未断句

每个错误的 reason 中，错误类型必须归类为以上 6 类之一，再加具体讲解。"""
  ...
  "errors": [
    {
      ...
      "reason": "的、地、得误用：「跳」是动词，前面修饰语应用「地」"
    },
    {
      "error_text": "开心的说不出话来",
      "correct_text": "开心地说不出话来",
      "reason": "的、地、得误用：「说不出话来」是动作状态，前面应用「地」"
    }
  ],
...
- 多字少字：句子中多余或遗漏的字
- 病句修改：成分残缺
- 标点规范：标点使用是否正确、断句是否得当

## 核心原则
1. 以学生视角批改，指出确定的错误，不苛求文采，不按成人写作标准拔高
2. error_text 必须是原文的精确子串，逐字符匹配（包括空格和标点）
3. 每个错误独立标注，不合并相邻错误
4. reason 用「错误类型 + 具体讲解」的形式；错误类型必须归类为以下之一：错别字 / 的、地、得误用 / 多字、少字 / 成分残缺 / 标点符号使用错误 / 断句错误；语气像老师辅导学生，简明易懂
5. 如果文本无错误，直接返回无错误结果
...
2. **按修改符号还原作者意图**：
   ...
   - 对调符号连接的内容 → 按调换后的顺序识别
   - 原位 / 上方重写的字 → 替代被涂掉的原字
3. **涂改本身不是错误，但涂改处不跳过**：划掉重写是作者已完成的自我修正，被划掉的原始字不算错误；但涂改处重写后的最终字仍需逐字检查错别字、病句，不得因该位置有涂改就跳过或漏查
...
5. 每个错误独立标注，不合并相邻错误
6. reason 用「错误类型 + 具体讲解」的形式；错误类型必须归类为以下之一：错别字 / 的、地、得误用 / 多字、少字 / 成分残缺 / 标点符号使用错误 / 断句错误
...
6. 涂改、划掉重写的位置，其最终保留的字必须逐字检查，不得因涂改而跳过或漏查
